...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.hid_to_class  = ((class_prob - data.targets ) * hid_output')./m;   % (y-t)*a /m
  ret.hid_to_class += model.hid_to_class  * wd_coefficient;
  
  ret.input_to_hid  = model.hid_to_class' * (class_prob - data.targets ) .* hid_output.*(1-hid_output) * data.inputs'/m;  % (y-t)*w_hs*a(1-a)*X
